...
2. **Diff the five sync-worthy folders** — compare each file between this project and the template:
...
5. **Apply changes** (after confirmation):
   - Copy changed files from `template-docs/`, `agents/`, `skills/`, `rules/`, `rules-for-skill/` to the template directory
